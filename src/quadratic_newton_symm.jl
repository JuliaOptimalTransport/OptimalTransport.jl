...
struct SymmetricQuadraticOTNewtonAS{T<:Real,K<:Real,D<:Real,ST<:AbstractSparseArray, IT, JT} <: QuadraticOT
    θ::T
    κ::K
    δ::D
    armijo_max::Int
...
Base.show(io::IO, ::SymmetricQuadraticOTNewtonAS) = print(io, "Symmetric semi-smooth Newton algorithm (active set)")

function SymmetricQuadraticOTNewton(; θ=0.1, κ=0.5, δ=1e-5, armijo_max=50)
    return SymmetricQuadraticOTNewton(θ, κ, δ, armijo_max)
end

function SymmetricQuadraticOTNewtonAS(S; θ=0.1, κ=0.5, δ=1e-5, armijo_max=50)
    I, J, V = findnz(S)
    return SymmetricQuadraticOTNewtonAS(θ, κ, δ, armijo_max, S, I, J)
end
...
function descent_dir!(solver::QuadraticOTSolver{<:SymmetricQuadraticOTNewtonAS})
    # unpack solver
    eps = solver.eps
    C = solver.C
    μ = solver.source
    ...
    # setup intermediate variables
    @. γ.nzval = u[I] + u[J] - C.nzval
    
    @. σ.nzval = γ.nzval ≥ 0
    @. γ.nzval = NNlib.relu(γ.nzval) / eps

    # setup kernel matrix G
    # G = Diagonal(vec(sum(σ; dims=2))) + σ + δ*I
    fill!(G.nzval, 0)
    G += σ
    G += Diagonal(vec(sum(σ; dims=2)) .+ δ) 
...
function descent_step!(solver::QuadraticOTSolver{<:SymmetricQuadraticOTNewtonAS})
    # unpack solver
    eps = solver.eps
    C = solver.C
    μ = solver.source
    ...
    function Φ(u, μ, C, ε, I, J)
        return norm(NNlib.relu.(u[I] + u[J] - C.nzval))^2 / 2 - 2*ε * dot(μ, u) 
    end

    # compute directional derivative
    d = -eps * (2*dot(δu, μ)) + eps * dot(γ.nzval, δu[I] + δu[J])
    t = 1
    Φ0 = Φ(u, μ, C, eps, I, J)
    while (armijo_counter < armijo_max) &&
        (Φ(u + t * δu, μ, C, eps, I, J) ≥ Φ0 + t * θ * d)
        t = κ * t
        armijo_counter += 1
    end
    return u .= u + t * δu
...
function solve!(solver::QuadraticOTSolver{<:SymmetricQuadraticOTNewtonAS})
    # unpack solver
    μ = solver.source
    atol = solver.atol
    rtol = solver.rtol
    ...
    function check_convergence_dual(u, u_prev, atol, rtol)
        ...
        isconverged =
            norm_diff <
            max(atol, rtol * max(norm(u), norm(u_prev)))
        return isconverged, norm_diff
    end
    ...
    for iter in 1:maxiter
        ...
        if to_check_step == 0 || iter == maxiter
            ...
            #     μ, μ, cache, convergence_cache, atol, rtol
            # )
            isconverged, abserror = check_convergence_dual(solver.cache.u, u_prev, atol, rtol)
            @debug string(solver.alg) *
                   " (" *
                   string(iter) *
                   "/" *
...
function build_cache(
    ::Type{T}, 
    alg::SymmetricQuadraticOTNewtonAS,
    μ::AbstractVector, 
    ν::AbstractVector, 
    C::AbstractMatrix,
    ε::Real
    ...
    N = size(ν, 1)
    # G = similar(u, T, M, M)
    G = similar(alg.S + sparse(SparseArrays.I, M, M), T)
    # initial guess for conjugate gradient 
    x = similar(u, T, M)
    fill!(x, zero(T))
    return QuadraticOTNewtonCache(u, v, δu, δv, σ, γ, G, x, M, N)
...
function quadreg(μ, C, ε, alg::SymmetricQuadraticOTNewtonAS; maxiter_as = 5, kwargs...)
    function check_convergence(γ, cache, convergence_cache, atol, rtol)
        norm_diff = norm(vec(sum(γ; dims=2)) .- μ, Inf)
        isconverged =
            norm_diff <
            max(atol, rtol * max(convergence_cache.norm_source, convergence_cache.norm_target))
    ...
    u = similar(μ)
    fill!(u, 0)
    for iter = 1:maxiter_as
        _alg = SymmetricQuadraticOTNewtonAS(alg.θ, alg.κ, alg.δ, alg.armijo_max, S, I, J)
        Csp_v = similar(C, length(_alg.I)); @inbounds for k = 1:length(_alg.I) Csp_v[k] = C[_alg.I[k], _alg.J[k]] end
        solver = build_solver(μ, sparse(_alg.I, _alg.J, Csp_v), ε, _alg; kwargs...)
        copy!(solver.cache.u, u)
        solve!(solver)
        copy!(u, solver.cache.u)
        γ = plan(solver, C)
        isconverged, norm_diff = check_convergence(γ, solver.cache, solver.convergence_cache, solver.atol, solver.rtol)
        if isconverged
            @debug "$(solver.alg) AS step ($iter/$maxiter_as): converged"
            break
        else
            S = max.(_alg.S, sign.(γ))
            I, J, _ = findnz(S)
            @debug "$(solver.alg) AS growing support to $(nnz(S)) (sparsity $(nnz(S)/length(S)))"
        end
    end
    return γ
end
...
function plan(solver::QuadraticOTSolver{<:SymmetricQuadraticOTNewtonAS}, Cfull)
    cache = solver.cache
    γ = sparse(NNlib.relu.(cache.u .+ cache.u' .- Cfull) / solver.eps)
    return γ
end

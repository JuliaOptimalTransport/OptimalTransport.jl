...
"""
    otPlan1d(c, μ::UnivariateDistribution, ν::UnivariateDistribution)

Calculate the optimal transport map of the Monge-Kantorovich problem with univariate distributions `μ` and `ν` as source and target marginals and cost function `c`.
Cost function `c` has to be of the form ``c(x, y) = h(|x - y|)`` where ``h`` is a
convex function.

...
@testset "dual.jl" begin
    ...
    @testset "batch" begin
        ...
        v = rand(size(ν)...)
        @testset "dual" begin
            @test Dual.ot_entropic_dual(u, v, ε, K) == [
                Dual.ot_entropic_dual(x, y, ε, K) for (x, y) in zip(eachcol(u), eachcol(v))
            ]

            ∇_u, ∇_v = Dual.ot_entropic_dual_grad(u, v, ε, K)
            grad_pairwise = [
                Dual.ot_entropic_dual_grad(x, y, ε, K) for
                (x, y) in zip(eachcol(u), eachcol(v))
            ]
            @test (∇_u ≈ hcat([g[1] for g in grad_pairwise]...)) && (∇_v ≈ hcat([g[2] for g in grad_pairwise]...))

            @test Dual.getprimal_ot_entropic_dual(u, v, ε, K) ≈ cat(
                [
                    Dual.getprimal_ot_entropic_dual(x, y, ε, K) for
                    (x, y) in zip(eachcol(u), eachcol(v))
                ]...;
                dims=3,
            )
        ...
        @testset "semidual" begin
            @test Dual.ot_entropic_semidual(μ, v, ε, K) == [
                Dual.ot_entropic_semidual(x, y, ε, K) for
                (x, y) in zip(eachcol(μ), eachcol(v))
            ]
            @test Dual.ot_entropic_semidual_grad(μ, v, ε, K) ≈ hcat(
                [
                    Dual.ot_entropic_semidual_grad(x, y, ε, K) for
                    (x, y) in zip(eachcol(μ), eachcol(v))
                ]...,
            )

            @test Dual.getprimal_ot_entropic_semidual(μ, v, ε, K) ≈ hcat(
                [
                    Dual.getprimal_ot_entropic_semidual(x, y, ε, K) for
                    (x, y) in zip(eachcol(μ), eachcol(v))
                ]...,
            )
        end

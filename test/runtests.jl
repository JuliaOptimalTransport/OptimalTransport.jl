    ...
    if GROUP == "All" || GROUP == "OptimalTransport"
        ...
        @testset "entropically regularized transport" begin
            ...
            @testset "example" begin
                ...
                c_w_regularization = sinkhorn2(
                    μ, ν, C, eps; maxiter=5_000, regularization=true
                )
            ...
            @testset "Float32" begin
                ...
                # create random cost matrix
                C = pairwise(
                    SqEuclidean(), rand(Float32, 1, M), rand(Float32, 1, N); dims=2
                )
        ...
        @testset "sinkhorn barycenter" begin
            @testset "example" begin
                ...
                μ_interp_pot = POT.barycenter(
                    μ_all', C, eps; weights=[0.5, 0.5], stopThr=1e-9
                )

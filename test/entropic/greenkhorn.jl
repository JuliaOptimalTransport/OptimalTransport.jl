...
@testset "sinkhorn_gibbs.jl" begin
    ...
    @testset "example" begin
        ...
        γ = sinkhorn(μ, ν, C, ε, Greeenkhorn(); maxiter=5_000, rtol=1e-9)
        c = sinkhorn2(μ, ν, C, ε, Greeenkhorn(); maxiter=5_000, rtol=1e-9)
        ...
        c_w_regularization = sinkhorn2(
            μ, ν, C, ε, Greeenkhorn(); maxiter=5_000, regularization=true
        )
        ...
        c2 = sinkhorn2(similar(μ), similar(ν), C, rand(), Greeenkhorn(); plan=γ)
        @test c2 ≈ c
        @test c2 == sinkhorn2(similar(μ), similar(ν), C, rand(); plan=γ)
        c2_w_regularization = sinkhorn2(
            similar(μ), similar(ν), C, ε, Greeenkhorn(); plan=γ, regularization=true
        )
        ...
        for (size2_μ, size2_ν) in
            ...
            γ_all = sinkhorn(
                μ_batch, ν_batch, C, ε, Greeenkhorn(); maxiter=5_000, rtol=1e-9
            )
    ...
    @testset "Float32" begin
        ...
        # compute optimal transport plan and optimal transport cost
        γ = sinkhorn(μ32, ν32, C32, ε32, Greeenkhorn(); maxiter=5_000, rtol=1e-6)
        c = sinkhorn2(μ32, ν32, C32, ε32, Greeenkhorn(); maxiter=5_000, rtol=1e-6)
        ...
        for (size2_μ, size2_ν) in
            ...
            # plan for individual histograms
            γ_all = sinkhorn(
                μ32_batch, ν32_batch, C32, ε32, Greeenkhorn(); maxiter=5_000, rtol=1e-6
            )
            @test size(γ_all) == (M, N, d)
            @test all(view(γ_all, :, :, i) ≈ γ for i in axes(γ_all, 3))
            @test γ_all ==
    ...
    @testset "AD" begin
        ...
        for Diff in [ReverseDiff, ForwardDiff]
            ∇ = Diff.gradient(log.(ν)) do xs
                sinkhorn2(μ, softmax(xs), C, ε, Greeenkhorn(); regularization=true)
            end
            ...
            solver = OptimalTransport.build_solver(μ, ν, C, ε, Greeenkhorn())
            ...
            ∇ = Diff.gradient(log.(μ)) do xs
                sinkhorn2(softmax(xs), ν, C, ε, Greeenkhorn(); regularization=true)
            end
